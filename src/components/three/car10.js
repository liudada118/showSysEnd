...
const Canvas = React.forwardRef((props, refs) => {
  ...
  let groupRect = new THREE.Group();

  let positions1, positionsRect;
  let colors1, scales1, scalesRect, colorsRect;
  let positions;
  let colors, scales;
  ...
  function initSet() {
    ...
    for (let ix = 0; ix < AMOUNTX; ix++) {
      for (let iy = 0; iy < AMOUNTY; iy++) {
        positions[i] = ix * (SEPARATION + 50) - (AMOUNTX * SEPARATION) / 2; // x
        positions[i + 1] = 0; // y
        positions[i + 2] = iy * (SEPARATION + 50) - (AMOUNTY * SEPARATION) / 2; // z
  ...
  //
  function initRect() {
    const numParticles = 2 * 10;
    positionsRect = new Float32Array(numParticles * 3);
    scalesRect = new Float32Array(numParticles);
    colorsRect = new Float32Array(numParticles * 3);
    let i = 0,
      j = 0;

    for (let ix = 0; ix < 5; ix++) {
      for (let iy = 0; iy < 5; iy++) {
        const RectGeometry = new THREE.PlaneGeometry(50, 100);
        ...
        const particlesRect = new THREE.Mesh(RectGeometry, Rectmaterial);
        groupRect.add(particlesRect)
        particlesRect.scale.x = 0.1;
        particlesRect.scale.y = 0.1;
        particlesRect.scale.z = 0.1;
    ...
    groupRect.rotation.x = -Math.PI / 24;
  ...
  function RectRenew() {
    ...
    //     num += ndata[i*5 + k]
    //   }
    //   newArr.push(Number((num / 5).toFixed(1)))
    // }
    ...
    // for (let i = 8; i < 10; i++) {
    //   for (let j = 0; j < 10; j++) {
    //     resArr.push(ndata[i * 10 + j])
    //   }
    // }

    // console.log(resArr)
    // console.log(groupRect.children[0].material.color)
    for (let i = 0; i < 25; i++) {
      // for(let j = 0 ; j < 2 ; j ++){
      groupRect.children[i].material.color.r = jetRgb(0, valuej1, newArr[i]).r
      groupRect.children[i].material.color.g = jetRgb(0, valuej1, newArr[i]).g
      groupRect.children[i].material.color.b = jetRgb(0, valuej1, newArr[i]).b
      // groupRect.children[i].position.x =  newArr[i]
  ...
  function changeRect(value) {
    if (value) {
      // initRect()
      // group.remove(particles1)
      particles1.visible = false
      groupRect.visible = true
    } else {
      // initBack()
      // group.remove(groupRect)
      // groupRect = new THREE.Group()
      groupRect.visible = false
      particles1.visible = true
    }
  }
